        ...
        <div class="p-4 sm:p-6">
            ...
            <div class="mb-8 bg-gray-50 dark:bg-gray-900 rounded-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
                ...
                <?php if($slides->where('is_active', true)->count() > 0): ?>
                    ...
                    <div x-data="{ current: 0, total: <?php echo e($activeSlides->count()); ?>, auto: null,
                        init() {
                            this.auto = setInterval(() => { this.current = (this.current + 1) % this.total }, 5000);
                        },
                        goto(i) { this.current = i; clearInterval(this.auto); this.auto = setInterval(() => { this.current = (this.current + 1) % this.total }, 5000); },
                        ...
                        <div class="relative" style="min-height: 400px;">
                            <?php $__currentLoopData = $activeSlides; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $slide): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div x-show="current === <?php echo e($index); ?>"
                                     ...
                                     class="absolute inset-0 flex items-center p-6 sm:p-10"
                                     style="background-color: <?php echo e($slide->background_color ?? '#6A0DAD'); ?>;">
                                    <div class="w-full">
                                        <div class="flex flex-col md:flex-row items-center gap-6 md:gap-10 <?php echo e($slide->image_position === 'left' ? 'md:flex-row-reverse' : ''); ?>">
                                            <!-- Texte -->
                                            <div class="flex-1 <?php echo e($slide->text_position === 'center' ? 'text-center' : ($slide->text_position === 'right' ? 'text-right' : 'text-left')); ?>">
                                                <h2 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-3 leading-tight"><?php echo e($slide->title); ?></h2>
                                                <?php if($slide->subtitle): ?>
                                                    <p class="text-base sm:text-lg text-white/85 mb-6"><?php echo e($slide->subtitle); ?></p>
                                                <?php endif; ?>
                                                <div class="flex flex-wrap gap-3 <?php echo e($slide->text_position === 'center' ? 'justify-center' : ($slide->text_position === 'right' ? 'justify-end' : 'justify-start')); ?>">
                                                    <?php if($slide->button_primary_text): ?>
                                                        <a href="<?php echo e($slide->button_primary_url ?? '#'); ?>" class="inline-flex items-center px-5 py-2.5 bg-white text-gray-900 font-semibold rounded-lg shadow-md hover:bg-gray-100 hover:shadow-lg transition-all duration-200">
                                                            <?php echo e($slide->button_primary_text); ?>

                                                        </a>
                                                    <?php endif; ?>
                                                    <?php if($slide->button_secondary_text): ?>
                                                        <a href="<?php echo e($slide->button_secondary_url ?? '#'); ?>" class="inline-flex items-center px-5 py-2.5 bg-transparent text-white font-semibold rounded-lg border-2 border-white/70 hover:bg-white/10 hover:border-white transition-all duration-200">
                                            ...
                                            <div class="flex-1 flex justify-center">
                                                <img src="/storage/<?php echo e($slide->image_url); ?>" 
                                                     ...
                                                     style="max-height: <?php echo e($slide->image_size === 'small' ? '250px' : ($slide->image_size === 'medium' ? '350px' : ($slide->image_size === 'large' ? '450px' : '100%'))); ?>;"
                        ...
                        <!-- Navigation flèches -->
                        <button @click="prev()" class="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/30 backdrop-blur-sm text-white flex items-center justify-center hover:bg-black/50 transition-all duration-200 z-10">
                            <i class="fas fa-chevron-left"></i>
                        </button>
                        <button @click="next()" class="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/30 backdrop-blur-sm text-white flex items-center justify-center hover:bg-black/50 transition-all duration-200 z-10">
                        ...
                        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 z-10">
                            <?php $__currentLoopData = $activeSlides; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $slide): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <button @click="goto(<?php echo e($index); ?>)"
                                        :class="current === <?php echo e($index); ?> ? 'w-8 bg-white' : 'w-3 bg-white/50 hover:bg-white/75'"
            ...
            <div class="space-y-4">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
                    ...
                    Toutes les Slides
                    <span class="ml-2 inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300"><?php echo e($slides->count()); ?></span>
                </h3>

                <?php if($slides->count() > 0): ?>
                    <div id="slidesList" class="space-y-3">
                        <?php $__currentLoopData = $slides; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slide): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="group bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 hover:shadow-lg hover:border-primary-300 dark:hover:border-primary-600 transition-all duration-300 slide-item" data-slide-id="<?php echo e($slide->id); ?>">
                                <div class="flex flex-col md:flex-row gap-4">
                                    <!-- Image -->
                                    <div class="flex-shrink-0 relative overflow-hidden rounded-lg">
                                        <img src="/storage/<?php echo e($slide->image_url); ?>" alt="<?php echo e($slide->title); ?>" class="w-full md:w-48 h-32 object-cover rounded-lg group-hover:scale-105 transition-transform duration-300">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Mes projets\vintApp\resources\views/admin/settings/hero-slides.blade.php ENDPATH**/ ?>