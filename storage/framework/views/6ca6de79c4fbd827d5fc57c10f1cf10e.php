<?php $__env->startSection('title', 'Articles disponibles'); ?>
        ...
        <div class="mb-6 md:mb-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <?php if(auth()->guard()->check()): ?>
                    <a href="<?php echo e(route('items.create')); ?>" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-primary-600 to-accent-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl active:scale-[0.98] transition-all">
            ...
            <div id="filtersModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
                <div class="flex items-end sm:items-center justify-center min-h-screen">
                    ...
                    <div class="relative w-full sm:max-w-lg bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
                        ...
                        <div class="px-6 py-5">
                            <form method="GET" action="<?php echo e(route('items.index')); ?>" id="filterForm" class="space-y-5">
                                <div>
        ...
        <?php if($items->count() > 0): ?>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5" id="items-grid">
                <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $loop_index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    ?>
                    <div class="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 <?php echo e($isBoosted ? 'ring-2 ring-' . ($boostType?->color ?? 'primary') . '-400/50' : ''); ?>">
                        
                        <a href="<?php echo e(route('items.show', $item)); ?>" class="block relative aspect-[4/3] overflow-hidden bg-gray-100 dark:bg-gray-700">
                            <?php if($isBoosted): ?>
                                <div class="absolute inset-0 bg-gradient-to-tr from-<?php echo e($boostType?->color ?? 'primary'); ?>-400/10 to-transparent pointer-events-none z-10"></div>
                            <?php endif; ?>
                            ...
                            <div class="absolute top-2 left-2 flex flex-col gap-1 z-20">
                                <?php if($isBoosted): ?>
                                    <span class="px-2 py-0.5 bg-gradient-to-r from-<?php echo e($boostType?->color ?? 'primary'); ?>-500 to-<?php echo e($boostType?->color ?? 'primary'); ?>-600 text-white text-[10px] md:text-xs font-bold rounded-full shadow-lg flex items-center gap-1 animate-pulse">
                                        <i class="<?php echo e($boostType?->icon ?? 'fas fa-star'); ?> text-[9px]"></i>
                                        <span class="hidden sm:inline"><?php echo e($boostType?->name ?? 'BOOST'); ?></span>
                                    </span>
                                <?php endif; ?>
                                <?php if($isNew): ?>
                            ...
                            <div class="absolute bottom-2 right-2 z-20">
                                <span class="px-2.5 py-1 bg-gray-900/80 backdrop-blur-sm text-white rounded-lg text-xs md:text-sm font-bold shadow-lg <?php echo e($isBoosted ? 'ring-1 ring-' . ($boostType?->color ?? 'primary') . '-400' : ''); ?>">
                        ...
                        <div class="p-3 md:p-4">
                            <a href="<?php echo e(route('items.show', $item)); ?>" class="block group/title">
                            ...
                            <div class="flex flex-wrap gap-1 mt-2">
                                <span class="px-1.5 md:px-2 py-0.5 text-[10px] md:text-xs font-medium bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 rounded"><?php echo e($item->category->name); ?></span>
                                <?php if($item->brand): ?>
                                    <span class="px-1.5 md:px-2 py-0.5 text-[10px] md:text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded"><?php echo e($item->brand->name); ?></span>
                                <?php endif; ?>
                            </div>

                            <p class="hidden md:block text-gray-500 dark:text-gray-400 text-xs mt-2 line-clamp-2">
                                <?php echo e(Str::limit($item->description, 70)); ?>
                            ...
                            <div class="flex items-center justify-between mt-2.5 md:mt-3 pt-2.5 md:pt-3 border-t border-gray-100 dark:border-gray-700">
                                <div class="flex items-center gap-1.5 min-w-0">
                                    ...
                                    <span class="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 truncate"><?php echo e($item->user->name); ?></span>
                                </div>
                                <div class="flex items-center gap-2 flex-shrink-0 text-[10px] md:text-xs text-gray-400 dark:text-gray-500">
                                    ...
                                    <span class="hidden sm:inline"><?php echo e($item->created_at->diffForHumans(null, true)); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/aizen/Bureau/sky/vintApp/resources/views/items/index.blade.php ENDPATH**/ ?>