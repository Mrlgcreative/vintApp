...
<div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 mb-6">
        <form method="GET" action="<?php echo e(route('agent.tickets')); ?>" class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
            
            <div class="col-span-2 sm:col-span-4 lg:col-span-1">
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Rechercher..."
            ...
            <div>
                <select name="status" class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500">
                    ...
                    <option value="waiting_user" <?php echo e(request('status') === 'waiting_user' ? 'selected' : ''); ?>>Attente utilisateur</option>
            ...
            <div class="flex gap-2">
                ...
                <a href="<?php echo e(route('agent.tickets')); ?>" class="px-3 py-2 text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">
    ...
    <?php else: ?>
        ...
        <div class="sm:hidden space-y-3">
            <?php $__currentLoopData = $chats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $chat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('agent.show', $chat)); ?>" class="block bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 hover:shadow-md transition-shadow">
                    <div class="flex items-start justify-between mb-2">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-gray-900 dark:text-white truncate"><?php echo e($chat->subject); ?></p>
                            <p class="text-xs text-gray-500 dark:text-gray-400"><?php echo e($chat->reference); ?></p>
                        </div>
                        <?php
                            $statusColors = [
                        ...
                        ?>
                        <span class="ml-2 inline-flex px-2 py-0.5 text-[10px] font-medium rounded-full <?php echo e($statusColors[$chat->status] ?? ''); ?>">
                            <?php echo e($statusLabels[$chat->status] ?? $chat->status); ?>

                        </span>
                    </div>
                    <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                        <span><i class="fas fa-user mr-1"></i><?php echo e($chat->user->name ?? '?'); ?></span>
                        <span class="<?php echo e($prioColors[$chat->priority] ?? ''); ?>"><?php echo e(ucfirst($chat->priority)); ?></span>
                        <span><?php echo e($chat->last_message_at?->diffForHumans()); ?></span>
                    </div>
                </a>
        ...
        <div class="hidden sm:block bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    <?php $__currentLoopData = $chats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $chat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer" onclick="window.location='<?php echo e(route('agent.show', $chat)); ?>'">
                            <td class="px-4 py-3">
                                <span class="text-sm font-mono text-gray-600 dark:text-gray-400"><?php echo e($chat->reference); ?></span>
                            </td>
                            <td class="px-4 py-3">
                                <p class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-[200px] lg:max-w-xs"><?php echo e($chat->subject); ?></p>
                                <p class="text-xs text-gray-500 dark:text-gray-400"><?php echo e(ucfirst($chat->category)); ?></p>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <div class="w-7 h-7 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center text-xs font-bold text-gray-600 dark:text-gray-300">
                                        <?php echo e(strtoupper(substr($chat->user->name ?? '?', 0, 2))); ?>

                                    </div>
                                    <span class="text-sm text-gray-700 dark:text-gray-300"><?php echo e($chat->user->name ?? '?'); ?></span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex px-2 py-0.5 text-[10px] font-medium rounded-full <?php echo e($statusColors[$chat->status] ?? ''); ?>">
                                    <?php echo e($statusLabels[$chat->status] ?? $chat->status); ?>
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                ?>
                                <span class="inline-flex px-2 py-0.5 text-[10px] font-medium rounded-full <?php echo e($prioBadge[$chat->priority] ?? ''); ?>">
                                    <?php echo e(ucfirst($chat->priority)); ?>
...
<?php echo $__env->make('layouts.agent', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Mes projets\vintApp\resources\views/agent/tickets.blade.php ENDPATH**/ ?>