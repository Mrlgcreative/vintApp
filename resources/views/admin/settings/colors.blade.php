        ...
        <div class="space-y-6 p-4 sm:p-6">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" id="paletteGrid">
                        @foreach(config('colors.palettes') as $paletteKey => $palette)
                            <div class="palette-card cursor-pointer group border-2 rounded-lg p-4 transition-all hover:shadow-md {{ ($activePaletteName ?? 'default') === $paletteKey ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300' }}"
                                 data-palette="{{ $paletteKey }}"
                                 onclick="changePalette('{{ $paletteKey }}')">
                                <!-- Nom de la palette -->
                                <h4 class="text-sm font-medium text-gray-900 mb-3">{{ $palette['name'] }}</h4>
                                
                                <!-- Aperçu des couleurs -->
                                <div class="flex gap-1 mb-3">
                                    <div class="w-6 h-6 rounded-sm border border-gray-200" style="background-color: {{ $palette['primary'] }}"></div>
                                    <div class="w-6 h-6 rounded-sm border border-gray-200" style="background-color: {{ $palette['secondary'] }}"></div>
                                    <div class="w-6 h-6 rounded-sm border border-gray-200" style="background-color: {{ $palette['success'] }}"></div>
                                    <div class="w-6 h-6 rounded-sm border border-gray-200" style="background-color: {{ $palette['danger'] }}"></div>
                                    <div class="w-6 h-6 rounded-sm border border-gray-200" style="background-color: {{ $palette['accent'] }}"></div>
                                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div id="customPaletteForm" class="hidden">
                    <form action="{{ route('admin.settings.colors.custom') }}" method="POST" class="space-y-4">
                        @csrf
                        ...
                        <div class="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-4">
                            @foreach(['primary', 'secondary', 'success', 'danger', 'warning', 'info', 'light', 'dark', 'accent'] as $colorName)
                                <div class="text-center">
                                    <label for="color_{{ $colorName }}" class="block text-sm font-medium text-gray-700 mb-1 capitalize">
                                        {{ $colorName }}
                                    </label>
                                    <input type="color" 
                                           name="{{ $colorName }}" 
...
function changePalette(paletteName) {
    ...
    fetch('{{ route("admin.settings.colors.update") }}', {
        method: 'POST',
        headers: {
