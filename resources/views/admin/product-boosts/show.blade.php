...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h4 class="font-semibold text-slate-900 dark:text-white mb-4">Informations générales</h4>
            <div class="space-y-3 text-sm">
                <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-700">
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                        ...
                        {{ $productBoost->status === 'expired' ? 'bg-slate-100 text-slate-800 border border-slate-200' : '' }}
                        ...
                        <span class="w-1.5 h-1.5 rounded-full mr-1.5
                            ...
                            {{ $productBoost->status === 'expired' ? 'bg-slate-500' : '' }}
                ...
                <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-700">
                    <span class="text-slate-500">Type de boost</span>
                ...
                <div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-700">
                    <span class="text-slate-500">Annulé le</span>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h4 class="font-semibold text-slate-900 dark:text-white mb-4">Performance</h4>
            <div class="grid grid-cols-2 gap-4">
                <div class="text-center p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($productBoost->views_generated) }}</p>
                    <p class="text-xs text-slate-500 mt-1">Vues générées</p>
                </div>
                <div class="text-center p-4 bg-slate-50 dark:bg-slate-900 rounded-lg">
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($productBoost->clicks_generated) }}</p>
                    <p class="text-xs text-slate-500 mt-1">Clics générés</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h4 class="font-semibold text-slate-900 dark:text-white mb-4">Article concerné</h4>
            @if($productBoost->item)
            <div class="flex items-center gap-4">
                <div class="w-16 h-16 rounded-lg bg-slate-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                ...
                <div>
                    <h5 class="font-semibold text-slate-900 dark:text-white">{{ $productBoost->item->name }}</h5>
                    <p class="text-sm text-slate-500">{{ Str::limit($productBoost->item->description ?? '', 100) }}</p>
                </div>
            </div>
            @else
            <p class="text-slate-500">Article non disponible.</p>
            @endif
        </div>

        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <h4 class="font-semibold text-slate-900 dark:text-white mb-4">Vendeur</h4>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h5 class="font-semibold text-slate-900 dark:text-white">{{ $productBoost->user->name }}</h5>
                    <p class="text-sm text-slate-500">{{ $productBoost->user->email }}</p>
                </div>
            </div>
            @else
            <p class="text-slate-500">Utilisateur non disponible.</p>
            @endif
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            ...
            <p class="text-sm text-slate-700 dark:text-slate-300">{{ $productBoost->cancellation_reason }}</p>
        </div>
        @endif
    </div>
</div>
...
<div id="cancelModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="flex items-center justify-center min-h-full p-4">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
            <button onclick="hideCancelModal()" class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors">
            ...
            <div class="text-center mb-6">
                ...
                <h3 class="text-xl font-bold text-slate-900 dark:text-white">Annuler ce boost ?</h3>
                <p class="text-sm text-slate-500 mt-2">Un remboursement partiel sera calculé et crédité sur le portefeuille du vendeur.</p>
            </div>
            <form action="{{ route('admin.product-boosts.cancel', $productBoost) }}" method="POST">
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="hideCancelModal()"
                            class="flex-1 px-4 py-2 border border-slate-300 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors">
                        Fermer
                    </button>
                    <button type="submit"
