...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <div>
        <h1 class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">Logs système</h1>
        <p class="text-slate-600 dark:text-slate-300 mt-1">Consultez et gérez les logs de l'application</p>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-slate-600 dark:text-slate-300">Erreurs aujourd'hui</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-slate-600 dark:text-slate-300">Avertissements</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-slate-600 dark:text-slate-300">Informations</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-slate-600 dark:text-slate-300">Taille du fichier</p>
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 mb-8">
    <div class="p-6">
        <h2 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">Filtres</h2>
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <!-- Niveau -->
            <div>
                <label for="level" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">
                    <i class="fas fa-layer-group text-slate-400 mr-1"></i>
                    Niveau
                </label>
                <select class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors bg-white dark:bg-slate-800" 
                        id="level" 
                        name="level">
            ...
            <!-- Date -->
            <div>
                <label for="date" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">
                    <i class="fas fa-calendar text-slate-400 mr-1"></i>
                    Date
                </label>
                <input type="date" 
                       class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors" 
                       id="date" 
                       name="date" 
                       value="{{ request('date') }}">
            </div>
            ...
            <!-- Bouton -->
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">&nbsp;</label>
                <button type="submit" 
...
<div class="hidden lg:block bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 dark:bg-slate-900">
        <h2 class="text-lg font-semibold text-slate-900 dark:text-white">
            <i class="fas fa-list mr-2 text-slate-600 dark:text-slate-300"></i>
            Entrées des logs
        </h2>
    </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                ...
                                $badgeClasses = match($levelLower) {
                                    ...
                                    'debug' => 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100',
    ...
    @forelse($logs as $log)
        @php
            ...
            $bgClass = match($levelLower) {
                ...
                'debug' => 'bg-slate-50 dark:bg-slate-900 border-slate-100',
                default => 'bg-slate-50 dark:bg-slate-900 border-slate-100',
            };
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div class="p-4 space-y-3">
                <div>
                    <p class="text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Message:</p>
                    <p class="text-sm text-slate-900 dark:text-white">{{ $log['message'] }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Contexte:</p>
                    <div class="text-sm text-slate-600 dark:text-slate-300 space-y-1">
                        <div><span class="font-medium">Env:</span> {{ $log['env'] }}</div>
                        @if(!empty(trim($log['context'])))
                            <div class="text-xs bg-slate-50 dark:bg-slate-900 p-2 rounded max-h-24 overflow-auto">
