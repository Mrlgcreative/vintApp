...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.support.index') }}" class="inline-flex items-center px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white font-medium rounded-lg transition-colors text-sm">
    ...
    <!-- Filtre par période -->
    <div class="mb-6">
        <div class="inline-flex rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-1 shadow-sm">
            @foreach([7 => '7 jours', 14 => '14 jours', 30 => '30 jours', 60 => '60 jours', 90 => '90 jours'] as $value => $label)
                <a href="{{ route('admin.support.stats', ['period' => $value]) }}"
                   class="px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium rounded-md transition-colors {{ (int)$period === $value ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4 mb-6">
        <!-- Total -->
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-blue-500 p-3 sm:p-4">
            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wide">Total Chats</p>
            <h3 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{{ $stats['overview']['total_chats'] }}</h3>
        </div>
        <!-- Ouverts -->
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-red-500 p-3 sm:p-4">
            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wide">Ouverts</p>
            <h3 class="text-xl sm:text-2xl font-bold text-red-600 dark:text-red-400">{{ $stats['overview']['open_chats'] }}</h3>
        </div>
        <!-- En cours -->
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-yellow-500 p-3 sm:p-4">
            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wide">En Cours</p>
            <h3 class="text-xl sm:text-2xl font-bold text-yellow-600 dark:text-yellow-400">{{ $stats['overview']['in_progress_chats'] }}</h3>
        </div>
        <!-- Fermés -->
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-green-500 p-3 sm:p-4">
            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wide">Fermés</p>
            <h3 class="text-xl sm:text-2xl font-bold text-green-600 dark:text-green-400">{{ $stats['overview']['closed_chats'] }}</h3>
        </div>
        <!-- Temps réponse moyen -->
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-purple-500 p-3 sm:p-4">
            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wide">Rép. Moy.</p>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-indigo-500 p-3 sm:p-4">
            <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wide">Résol. Moy.</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <div class="p-4 sm:p-6">
                ...
                @else
                    <p class="text-slate-500 dark:text-slate-400 text-sm text-center py-4">Aucune donnée pour cette période</p>
                @endif
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        ...
        <div class="p-4 sm:p-6">
            @if($stats['admin_performance']->count() > 0)
                        ...
                        <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="font-medium text-sm text-slate-900 dark:text-white">{{ $admin->name }}</span>
                                </div>
                ...
                <div class="hidden lg:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        <thead>
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Agent</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Chats Assignés</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Chats Résolus</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Taux de Résolution</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="font-medium text-slate-900 dark:text-white text-sm">{{ $admin->name }}</p>
            ...
            @else
                <p class="text-slate-500 dark:text-slate-400 text-sm text-center py-4">Aucun agent trouvé</p>
            @endif
        </div>
    </div>
</div>
