...
<div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-blue-500 p-4">
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Total agents</p>
            <h3 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $agents->count() }}</h3>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-green-500 p-4">
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Actifs</p>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-yellow-500 p-4">
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Tickets en cours</p>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-l-4 border-purple-500 p-4">
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Total résolus</p>
    ...
    @if($agents->isEmpty())
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-8 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-2">Aucun agent configuré</h3>
            <p class="text-slate-500 dark:text-slate-400 mb-4">Ajoutez des agents pour gérer les tickets de support.</p>
    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            @foreach($agents as $agent)
                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden agent-card" data-agent-id="{{ $agent->id }}">
                    {{-- Header agent --}}
                    <div class="p-4 sm:p-5">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center gap-3">
                                <div class="relative">
                                    ...
                                    <span class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-white dark:border-slate-800 {{ $agent->is_active ? 'bg-green-500' : 'bg-slate-400' }}"></span>
                                </div>
                                <div>
                            ...
                            <div class="relative" x-data="{ open: false }">
                                <button @click="open = !open" class="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg transition-colors">
                                    <i class="fas fa-ellipsis-v"></i>
                                </button>
                                <div x-show="open" @click.away="open = false" x-transition
                                     class="absolute right-0 mt-1 w-44 bg-white dark:bg-slate-700 rounded-lg shadow-lg border border-slate-200 dark:border-slate-600 py-1 z-10">
                                    <button onclick="editAgent({{ $agent->id }}, {{ $agent->max_chats }}, {{ json_encode($agent->specialties ?? []) }})" @click="open = false"
                                            class="w-full text-left px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors">
                                        <i class="fas fa-cog mr-2 text-slate-400"></i>Paramètres
                                    </button>
                                    <button onclick="toggleAgent({{ $agent->id }})" @click="open = false"
                                            class="w-full text-left px-3 py-2 text-sm {{ $agent->is_active ? 'text-yellow-600' : 'text-green-600' }} hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors">
                        ...
                        <div class="grid grid-cols-2 gap-2 mb-3">
                            <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg px-3 py-2 text-center">
                        ...
                        @if(!empty($agent->specialties))
                            <div class="flex flex-wrap gap-1.5">
                                @foreach($agent->specialties as $spec)
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 text-[10px] font-medium rounded-full {{ $specColors[$spec] ?? 'bg-slate-100 text-slate-700' }}">
                                        {{ $specLabels[$spec] ?? ucfirst($spec) }}
                                    </span>
                                @endforeach
                            </div>
                        @else
                            <p class="text-xs text-slate-400 italic">Aucune spécialité</p>
                        @endif
                    </div>

                    {{-- Footer --}}
                    <div class="px-4 sm:px-5 py-2.5 bg-slate-50 dark:bg-slate-700/30 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        <span class="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full {{ $agent->is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400' }}">
                            <span class="w-1.5 h-1.5 rounded-full {{ $agent->is_active ? 'bg-green-500' : 'bg-slate-400' }} mr-1.5"></span>
...
<div id="addAgentModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 py-8">
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg mx-4">
            <form id="addAgentForm">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                    <h3 class="text-lg font-semibold text-slate-900 dark:text-white">
                        <i class="fas fa-user-plus mr-2 text-blue-600"></i>Ajouter un agent
                    </h3>
                    <button type="button" onclick="closeAddModal()" class="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg">
                ...
                <div class="px-6 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <select id="agentUserId" required class="w-full px-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    {{-- Max tickets --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Tickets max simultanés</label>
                        <input type="number" id="agentMaxChats" value="10" min="1" max="50"
                               class="w-full px-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <p class="text-xs text-slate-500 mt-1">L'agent ne recevra plus de tickets au-delà de cette limite</p>
                    </div>

                    {{-- Spécialités --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Spécialités</label>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            ...
                            @foreach($specLabels as $key => [$label, $icon, $color])
                                <label class="flex items-center gap-2 p-2 border border-slate-200 dark:border-slate-600 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50 dark:has-[:checked]:bg-blue-900/20">
                                    <input type="checkbox" name="specialties[]" value="{{ $key }}" class="agent-specialty rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                ...
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 dark:border-slate-700">
                    <button type="button" onclick="closeAddModal()" class="px-4 py-2 text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg font-medium transition-colors">
                        Annuler
                    </button>
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg mx-4">
            <form id="editAgentForm">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <button type="button" onclick="closeEditModal()" class="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                ...
                <div class="px-6 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            @foreach($specLabels as $key => [$label, $icon, $color])
                                <label class="flex items-center gap-2 p-2 border border-slate-200 dark:border-slate-600 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50 dark:has-[:checked]:bg-blue-900/20">
                                    <input type="checkbox" name="edit_specialties[]" value="{{ $key }}" class="edit-specialty rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                                    <i class="fas {{ $icon }} text-xs text-{{ $color }}-500"></i>
                                    <span class="text-sm text-slate-700 dark:text-slate-300">{{ $label }}</span>
                                </label>
                            @endforeach
                ...
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 dark:border-slate-700">
                    <button type="button" onclick="closeEditModal()" class="px-4 py-2 text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg font-medium transition-colors">
                        Annuler
                    </button>
                    <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
