    ...
    <div class="p-6">
        <form method="GET" action="{{ route('admin.boost-types.index') }}" class="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('admin.boost-types.index') }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 dark:bg-gray-900 transition-colors">
                    <i class="fas fa-times"></i>
                </a>
            </div>
        </form>
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $boostTypes->total() }}</p>
                </div>
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $boostTypes->where('is_active', true)->count() }}</p>
                </div>
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $boostTypes->where('is_premium', true)->count() }}</p>
                </div>
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ \App\Models\ProductBoost::where('status', 'active')->count() }}</p>
                </div>
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
    <div class="p-4 md:p-6 border-b border-gray-200 dark:border-gray-700">
        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
                <h5 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-1">Liste des types de boost</h5>
                <p class="text-xs md:text-sm text-gray-500 dark:text-gray-400">{{ $boostTypes->total() }} type(s)</p>
            </div>
        </div>
    </div>
    <div class="p-0">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Nom</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Prix</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Durées</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Statut</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Boosts</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Ordre</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 dark:bg-gray-900 transition-colors {{ !$bt->is_active ? 'opacity-60' : '' }}">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-semibold text-gray-900 dark:text-white">{{ $bt->display_name }}</div>
                                    <div class="text-xs text-gray-500">({{ $bt->name }})</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-semibold text-gray-900 dark:text-white">${{ number_format($bt->price_usd ?? 0, 2) }}</div>
                            <div class="text-xs text-gray-500">{{ number_format($bt->price_cdf ?? 0, 0, ',', ' ') }} FC</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-1">
                                @foreach($bt->available_durations ?? [] as $d)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200">{{ $d }}j</span>
                                @endforeach
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $bt->is_active ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-gray-100 text-gray-800 border border-gray-200' }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $bt->is_active ? 'bg-green-500' : 'bg-gray-500' }} mr-1.5"></span>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500">{{ $bt->sort_order }}</td>
                        <td class="px-6 py-4">
        ...
        <div class="text-center py-12">
            ...
            <h5 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Aucun type de boost</h5>
            <p class="text-gray-500 dark:text-gray-400 mb-4">Créez votre premier type de boost.</p>
        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            ...
            <div class="bg-white dark:bg-gray-800 px-6 py-4">
                <p class="text-gray-700 dark:text-gray-200 mb-4">
                    Êtes-vous sûr de vouloir supprimer <strong class="text-gray-900 dark:text-white" id="deleteItemName"></strong> ?
                </p>
            ...
            <div class="bg-gray-50 dark:bg-gray-900 px-6 py-4 flex flex-col-reverse sm:flex-row justify-end gap-3">
                <button type="button" onclick="closeDeleteModal()" class="inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-50 transition-colors w-full sm:w-auto">
                    Annuler
                </button>
