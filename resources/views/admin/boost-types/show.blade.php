...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['active_count'] }}</p>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
    ...
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    <h4 class="font-bold text-gray-900 dark:text-white text-lg">{{ $boostType->display_name }}</h4>
                    <p class="text-sm text-gray-500">({{ $boostType->name }})</p>
                </div>
            </div>
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <span class="font-semibold {{ $boostType->is_active ? 'text-green-600' : 'text-gray-400' }}">
                ...
                <div class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                    <span class="text-gray-500">Prix USD</span>
                    <span class="font-semibold">${{ number_format($boostType->price_usd ?? 0, 2) }}</span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                    <span class="text-gray-500">Prix CDF</span>
                ...
                <div class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                    <span class="text-gray-500">Durées</span>
                    <span class="font-semibold">
                        @foreach($boostType->available_durations ?? [] as $d)
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-900 mr-1">{{ $d }}j</span>
                        @endforeach
                    </span>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $boostType->description }}</p>
        </div>
        @endif

        @if($boostType->benefits)
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h4 class="font-semibold text-gray-900 dark:text-white mb-3">Avantages</h4>
            <ul class="space-y-2">
    ...
    <div class="lg:col-span-2">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div class="p-4 md:p-6 border-b border-gray-200 dark:border-gray-700">
                <h5 class="text-lg font-bold text-gray-900 dark:text-white">Boosts récents</h5>
            </div>
            <div class="p-0">
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 dark:bg-gray-900 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-200">{{ $pb->user?->name ?? 'N/A' }}</td>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                                        {{ $pb->status === 'active' ? 'bg-green-100 text-green-800' : ($pb->status === 'expired' ? 'bg-gray-100 text-gray-800' : 'bg-red-100 text-red-800') }}">
                                        {{ ucfirst($pb->status) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $pb->created_at->format('d/m/Y') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="text-center py-8">
                    <p class="text-gray-500">Aucun boost pour ce type.</p>
                </div>
                @endif
            </div>
        </div>
