...
<div class="min-h-screen bg-slate-50 dark:bg-slate-900 p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-slate-600 dark:text-slate-300 mt-1">Sélectionnez des utilisateurs pour les désigner comme experts</p>
        </div>
        <div>
            <a href="{{ route('admin.experts.index') }}" 
                class="bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-slate-600 dark:text-slate-300">Total Candidats</p>
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $candidates->total() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-slate-600 dark:text-slate-300">Vérifiés</p>
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $candidates->where('email_verified_at', '!=', null)->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-slate-600 dark:text-slate-300">Actifs récemment</p>
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $candidates->where('last_activity', '>', now()->subDays(30))->count() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        <div class="p-6 border-b border-slate-200 dark:border-slate-700">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Filtrer les candidats</h3>
        </div>
        <div class="p-6">
            <form method="GET" action="{{ route('admin.experts.candidates') }}">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label for="min_orders" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Min. commandes</label>
                    ...
                    <div>
                        <label for="verified_only" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Statut</label>
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('admin.experts.candidates') }}" 
                               class="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">
        ...
        <div class="p-6">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-slate-200 dark:border-slate-700">
                                <th class="text-left py-3 px-4 font-semibold text-slate-700 dark:text-slate-200">Utilisateur</th>
                                <th class="text-center py-3 px-4 font-semibold text-slate-700 dark:text-slate-200">Commandes</th>
                                <th class="text-center py-3 px-4 font-semibold text-slate-700 dark:text-slate-200">Taux satisfaction</th>
                                <th class="text-center py-3 px-4 font-semibold text-slate-700 dark:text-slate-200">Dernière activité</th>
                                ...
                                <tr class="border-b border-slate-100 hover:bg-slate-50 dark:bg-slate-900">
                                    <td class="py-4 px-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-semibold text-slate-900 dark:text-white">{{ $candidate->name }}</div>
                                                <div class="text-sm text-slate-500 dark:text-slate-400">{{ $candidate->email }}</div>
                                                @if($candidate->phone)
                                                    <div class="text-xs text-slate-400">{{ $candidate->phone }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4 text-center">
                                        <div class="font-bold text-slate-900 dark:text-white">{{ $candidate->orders_count ?? 0 }}</div>
                                        <div class="text-xs text-slate-500 dark:text-slate-400">commandes</div>
                                    </td>
                                    <td class="py-4 px-4 text-center">
                                    ...
                                    <td class="py-4 px-4 text-center">
                                        ...
                                        @else
                                            <span class="text-slate-400">Inconnue</span>
                                        @endif
                                    </td>
                                    <td class="py-4 px-4 text-center">
            ...
            @else
                <div class="text-center py-12">
                    <i class="fas fa-users text-slate-400 text-6xl mb-4"></i>
                    <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-2">Aucun candidat trouvé</h3>
                    <p class="text-slate-600 dark:text-slate-300 mb-6">Aucun utilisateur ne correspond aux critères de sélection.</p>
                    <a href="{{ route('admin.experts.candidates') }}" 
...
<div id="designateModal" class="fixed inset-0 bg-slate-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-10 mx-auto p-5 border max-w-2xl shadow-lg rounded-lg bg-white dark:bg-slate-800">
        <div class="mt-3">
            <!-- En-tête -->
            <div class="text-center mb-6">
                ...
                <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">Désigner comme Expert</h3>
            ...
            <form id="designateForm">
                ...
                <div class="mb-6">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Niveau de Certification</label>
                    <div class="grid grid-cols-3 gap-3">
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-slate-50 dark:bg-slate-900">
                            <input type="radio" name="certification_level" value="junior" class="mr-3" checked>
                            <div>
                                <div class="font-medium text-slate-900 dark:text-white">Junior</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">Débutant</div>
                            </div>
                        </label>
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-slate-50 dark:bg-slate-900">
                            <input type="radio" name="certification_level" value="senior" class="mr-3">
                            <div>
                                <div class="font-medium text-slate-900 dark:text-white">Senior</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">Expérimenté</div>
                            </div>
                        </label>
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-slate-50 dark:bg-slate-900">
                            <input type="radio" name="certification_level" value="master" class="mr-3">
                            <div>
                                <div class="font-medium text-slate-900 dark:text-white">Master</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">Expert</div>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <!-- Biographie -->
                <div class="mb-6">
                    <label for="bio" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
                        Biographie / Notes (optionnel)
                    </label>
                    <textarea name="bio" id="bio" rows="3" 
                              placeholder="Ajoutez des informations sur l'expertise de cet utilisateur..."
                              class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></textarea>
                </div>

                <!-- Boutons -->
                <div class="flex gap-3">
                    <button type="button" onclick="closeModal()" 
                            class="flex-1 px-4 py-2 bg-slate-300 text-slate-800 dark:text-slate-100 text-base font-medium rounded-lg shadow-sm hover:bg-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-500 transition-colors">
                        Annuler
                    </button>
                    <button type="submit" 
...
document.querySelectorAll('input[name="certification_level"]').forEach(function(radio) {
    radio.addEventListener('change', function() {
        ...
        document.querySelectorAll('input[name="certification_level"]').forEach(function(r) {
            ...
            r.closest('label').classList.add('border-slate-300 dark:border-slate-600');
        });
        
        // Ajouter la classe active au label sélectionné
        if (this.checked) {
            this.closest('label').classList.add('border-blue-500', 'bg-blue-50');
            this.closest('label').classList.remove('border-slate-300 dark:border-slate-600');
        }
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (defaultRadio) {
        ...
        defaultRadio.closest('label').classList.remove('border-slate-300 dark:border-slate-600');
    }
