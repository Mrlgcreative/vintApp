    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow-lg p-8">
            ...
            <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                <p class="text-green-800">✅ Connecté en tant que: <strong>{{ Auth::user()->name }}</strong> (ID: {{ Auth::id() }})</p>
                @if(Auth::user()->fcm_token)
                <p class="text-green-700 text-sm mt-2">📱 Token FCM enregistré: {{ Str::limit(Auth::user()->fcm_token, 50) }}</p>
                @else
            ...
            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <a href="{{ route('login') }}" class="text-red-600 underline">Se connecter</a>
            </div>
            @endauth
        </div>
    </div>
