    ...
    <div class="w-full max-w-md">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-8 text-center animate-slide-up relative overflow-hidden">
            ...
            <div class="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4 mb-5 text-sm text-left space-y-2">
                ...
                @if(isset($amount) && $amount > 0)
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold text-gray-900 dark:text-white text-right">
                            ...
                            @if(isset($currency))
                                @if($currency === 'USD')
                                    <span class="text-gray-400 font-normal text-xs block">({{ number_format($amount * 2650, 0) }} CDF)</span>
                                @elseif($currency === 'CDF')
                                    <span class="text-gray-400 font-normal text-xs block">({{ number_format($amount / 2650, 2) }} USD)</span>
                                @endif
                            @else
                                <span class="text-gray-400 font-normal text-xs block">({{ number_format($amount * 2650, 0) }} CDF)</span>
                            @endif
                ...
                @if(isset($provider) && $provider)
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $provider }}</span>
                    </div>
                @endif

                <div class="flex justify-between">
                    <span class="text-gray-500">Date</span>
                    <span class="font-semibold text-gray-900 dark:text-white">{{ now()->format('d/m/Y à H:i') }}</span>
                </div>
            ...
            <div class="flex flex-col gap-2.5 mt-5">
                <a href="{{ route('cart.pay') }}" class="w-full px-5 py-2.5 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors text-center">
                    Réessayer le Paiement
                </a>
                <a href="{{ route('support.index') }}" class="w-full px-5 py-2.5 text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors text-center">
                    Contacter le Support
                </a>
                <a href="{{ route('dashboard') }}" class="w-full px-5 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors text-center">
                    Retour au Dashboard
                </a>
            </div>
