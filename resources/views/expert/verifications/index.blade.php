...
<div class="min-h-screen bg-gray-50 p-6">
    ...
    <div class="bg-white rounded-xl p-6 mb-8 shadow-md">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="text-center">
                ...
                <div class="text-sm text-gray-500 uppercase tracking-wider">En attente</div>
            </div>
            <div class="text-center">
                <div class="text-2xl font-bold text-green-600 mb-1">
                    {{ $verifications->where('status', 'expert_approved')->count() }}
                </div>
                <div class="text-sm text-gray-500 uppercase tracking-wider">Approuvées</div>
            </div>
            <div class="text-center">
                <div class="text-2xl font-bold text-red-600 mb-1">
                    {{ $verifications->where('status', 'expert_rejected')->count() }}
                </div>
                <div class="text-sm text-gray-500 uppercase tracking-wider">Rejetées</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 mb-8">
        ...
        @if(request()->hasAny(['status', 'category']))
            <div class="mt-4 flex flex-wrap gap-2">
                <strong class="text-gray-700">Filtres actifs:</strong>
                @if(request('status'))
    ...
    @forelse($verifications as $verification)
        ...
        <div class="bg-white rounded-xl shadow-md mb-6 overflow-hidden hover:shadow-lg transition-shadow border-l-4 {{ $priorityClass }}">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div class="lg:col-span-6 space-y-2">
                        <h5 class="font-semibold text-gray-900">{{ $verification->item->name ?? $verification->item->title ?? 'Produit sans nom' }}</h5>
                ...
                @if($verification->expert_notes)
                    <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                        <strong class="text-gray-900">Notes de l'expert :</strong>
                        <p class="mt-1 text-gray-700">{{ $verification->expert_notes }}</p>
                    </div>
                @endif
            </div>
        </div>
    @empty
        <div class="bg-white rounded-xl shadow-md p-8 text-center">
            ...
            <h4 class="text-xl font-semibold text-gray-900 mb-2">Aucune vérification trouvée</h4>
