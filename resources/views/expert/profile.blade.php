        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center">
            ...
            <div class="mt-4 lg:mt-0">
                <a href="{{ route('expert.dashboard') }}" 
                   class="bg-white dark:bg-gray-800 bg-opacity-20 hover:bg-opacity-30 text-white px-6 py-2 rounded-lg transition-all duration-300 inline-flex items-center">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
                <div class="bg-gradient-to-br from-gray-50 to-gray-100 p-8 text-center border-b border-gray-200 dark:border-gray-700">
                    ...
                    <p class="text-gray-600 dark:text-gray-300 mb-4">{{ $expert->email }}</p>
                    ...
                    @if($expertProfile->specialties ?? [])
                        <div class="flex flex-wrap justify-center gap-2">
                            @foreach($expertProfile->specialties as $specialty)
                                <span class="bg-gray-100 dark:bg-gray-800 border border-gray-300 px-3 py-1 rounded-full text-sm text-gray-700 dark:text-gray-200">
                                    {{ ucfirst($specialty) }}
                                </span>
                            @endforeach
                        </div>
                ...
                <div class="p-8 space-y-6">
                    <div class="bg-gray-50 dark:bg-gray-900 border-l-4 border-indigo-500 p-4 rounded-lg">
                        ...
                        <p class="text-gray-900 dark:text-white font-medium">{{ $expertProfile->created_at->format('d/m/Y') }}</p>
                        <p class="text-gray-500 dark:text-gray-400 text-sm">{{ $expertProfile->created_at->diffForHumans() }}</p>
                    </div>

                    <div class="bg-gray-50 dark:bg-gray-900 border-l-4 border-yellow-500 p-4 rounded-lg">
                        <div class="flex items-center text-yellow-600 font-semibold mb-2">
                            <i class="fas fa-clock mr-2"></i>
                            Temps moyen d'examen
                        </div>
                        <p class="text-gray-900 dark:text-white font-medium">{{ $detailedStats['avg_review_time'] }} minutes</p>
                    </div>

                    @if($expertProfile->bio)
                        <div class="bg-gray-50 dark:bg-gray-900 border-l-4 border-blue-500 p-4 rounded-lg">
                            <div class="flex items-center text-blue-600 font-semibold mb-2">
                                <i class="fas fa-info-circle mr-2"></i>
                                Biographie
                            </div>
                            <p class="text-gray-700 dark:text-gray-200">{{ $expertProfile->bio }}</p>
                        </div>
                    @endif

                    @if($expertProfile->credentials)
                        <div class="bg-gray-50 dark:bg-gray-900 border-l-4 border-green-500 p-4 rounded-lg">
                            <div class="flex items-center text-green-600 font-semibold mb-2">
                                <i class="fas fa-certificate mr-2"></i>
                                Certifications
                            </div>
                            <p class="text-gray-700 dark:text-gray-200">{{ $expertProfile->credentials }}</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-900 border-l-4 border-yellow-500 p-6 rounded-lg text-center">
                        <div class="text-3xl font-bold text-yellow-600 mb-2">{{ $detailedStats['pending'] }}</div>
                        <div class="text-gray-600 dark:text-gray-300 font-medium uppercase text-sm tracking-wide">En attente</div>
                    </div>

                    <div class="bg-gray-50 dark:bg-gray-900 border-l-4 border-green-500 p-6 rounded-lg text-center">
                        <div class="text-3xl font-bold text-green-600 mb-2">{{ $detailedStats['approved'] }}</div>
                        <div class="text-gray-600 dark:text-gray-300 font-medium uppercase text-sm tracking-wide">Approuvées</div>
                    </div>

                    <div class="bg-gray-50 dark:bg-gray-900 border-l-4 border-red-500 p-6 rounded-lg text-center">
                        <div class="text-3xl font-bold text-red-600 mb-2">{{ $detailedStats['rejected'] }}</div>
                        <div class="text-gray-600 dark:text-gray-300 font-medium uppercase text-sm tracking-wide">Rejetées</div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg">
                        ...
                        <div class="flex items-center">
                            <div class="flex-grow bg-gray-200 dark:bg-gray-700 rounded-full h-3 mr-4">
                                <div class="bg-green-500 h-3 rounded-full transition-all duration-1000" 
                                     style="width: {{ $expertProfile->approval_rate ?? 0 }}%"></div>
                            </div>
                            <span class="text-xl font-bold text-gray-900 dark:text-white">{{ number_format($expertProfile->approval_rate ?? 0, 1) }}%</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg">
                        ...
                        <div class="flex items-center">
                            ...
                            <span class="text-xl font-bold text-gray-900 dark:text-white">{{ number_format($qualityScore, 1) }}%</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
                <h4 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 flex items-center">
                    <i class="fas fa-edit mr-3 text-gray-600 dark:text-gray-300"></i>
                    Modifier le profil
                </h4>
                ...
                <form action="{{ route('expert.profile.update') }}" method="POST" 
                      class="bg-gray-50 dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                    @csrf
                    @method('PUT')

                    <div class="space-y-6">
                        <div>
                            <label for="bio" class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2 flex items-center">
                        ...
                        <div>
                            <label for="credentials" class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2 flex items-center">
                                <i class="fas fa-certificate mr-2"></i>
