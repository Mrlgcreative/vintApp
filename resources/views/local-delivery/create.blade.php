                    ...
                    <select name="order_id" id="order_id" required 
                        ...
                        @foreach($orders as $order)
                            <option value="{{ $order->id }}" 
                                    ...
                                    data-buyer-email="{{ $order->buyer->email }}"
                                    data-buyer-address="{{ $order->buyer_address_data['address'] }}"
                                    data-buyer-phone="{{ $order->buyer_address_data['phone'] }}"
                                    data-buyer-latitude="{{ $order->buyer_address_data['latitude'] }}"
                                    data-buyer-longitude="{{ $order->buyer_address_data['longitude'] }}"
                                    data-has-coordinates="{{ $order->buyer_address_data['has_coordinates'] ? 'true' : 'false' }}"
                                    data-source="{{ $order->buyer_address_data['source'] }}"
                                    {{ request('order_id') == $order->id ? 'selected' : '' }}>
                                Commande #{{ $order->order_number }} - {{ $order->buyer->name }}
                                @if($order->buyer_address_data['has_coordinates'])
