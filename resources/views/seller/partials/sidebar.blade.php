    ...
    <div class="p-6">
        <div class="flex items-center gap-3 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ Auth::user()->name }}</p>
            </div>
        </div>
        ...
        <nav class="space-y-1">
            ...
            <a href="{{ route('seller.reviews') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('seller.reviews') ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:text-gray-900 dark:hover:text-gray-200' }}">
                <i class="fas fa-star w-5 text-center"></i> Mes notes
            </a>
        </nav>

        <div class="mt-8 pt-6 border-t border-gray-100 dark:border-gray-700">
            <a href="{{ route('items.create') }}" class="flex items-center justify-center gap-2 w-full px-4 py-3 bg-primary text-white rounded-xl font-semibold hover:bg-primary-600 transition-colors shadow-lg shadow-primary-500/20">
                <i class="fas fa-plus"></i> Nouvel article
            </a>
            <a href="{{ url('/') }}" class="flex items-center justify-center gap-2 w-full px-4 py-3 mt-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-xl font-medium hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors text-sm">
...
<div id="seller-drawer" class="lg:hidden fixed top-0 left-0 z-[60] h-full w-72 max-w-[85vw] bg-white dark:bg-gray-800 shadow-2xl border-r border-gray-200 dark:border-gray-700 transform -translate-x-full transition-transform duration-300 ease-in-out overflow-y-auto">
    <div class="p-6">
        ...
        <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
            <a href="{{ route('items.create') }}" class="flex items-center justify-center gap-2 w-full px-4 py-3 bg-primary text-white rounded-xl font-semibold hover:bg-primary-600 transition-colors shadow-lg shadow-primary-500/20 text-sm" onclick="closeDrawer()">
                <i class="fas fa-plus"></i> Nouvel article
            </a>
            <a href="{{ url('/') }}" class="flex items-center justify-center gap-2 w-full px-4 py-3 mt-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-xl font-medium hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors text-sm" onclick="closeDrawer()">
                <i class="fas fa-arrow-left"></i> Retour à l'accueil
            </a>
        </div>
    </div>
