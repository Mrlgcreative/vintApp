...
<div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border-l-4 border-orange-500 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold {{ $stats['waiting_reply'] > 0 ? 'text-orange-600' : 'text-gray-900 dark:text-white' }}">{{ $stats['waiting_reply'] }}</h3>
                </div>
        ...
        {{-- Non assignés --}}
        <a href="{{ route('agent.unassigned') }}" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border-l-4 border-red-500 p-4 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Non assignés</p>
                    <h3 class="text-2xl font-bold {{ $stats['unassigned'] > 0 ? 'text-red-600' : 'text-gray-900 dark:text-white' }}">{{ $stats['unassigned'] }}</h3>
                    ...
                    @foreach($recentTickets as $ticket)
                        <a href="{{ route('agent.show', $ticket) }}" class="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <div class="flex-shrink-0 flex flex-col items-end gap-1">
                                ...
                                @endphp
                                <span class="inline-flex px-2 py-0.5 text-[10px] font-medium rounded-full {{ $statusColors[$ticket->status] ?? '' }}">
                    ...
                    @foreach($urgentUnassigned as $ticket)
                        <div class="flex items-center gap-3 px-5 py-3">
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0">
                                ...
                                @endphp
                                <span class="inline-flex px-2 py-0.5 text-[10px] font-medium rounded-full {{ $prioColors[$ticket->priority] ?? '' }}">
                                    {{ $prioLabels[$ticket->priority] ?? $ticket->priority }}
                                </span>
                                <button onclick="claimTicket({{ $ticket->id }})" class="inline-flex items-center px-2.5 py-1 text-xs font-medium bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors">
