...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('boostForm').addEventListener('submit', function(e) {
        ...
        .catch(error => {
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
                showNotification('Impossible de contacter le serveur. Vérifiez votre connexion internet.', 'error');
            } else if (error.message.includes('HTTP:')) {
                showNotification(`Erreur serveur lors de l'achat: ${error.message}`, 'error');
            } else {
                showNotification('Une erreur inattendue est survenue lors de l\'achat.', 'error');
            }
        })
        .finally(() => {
