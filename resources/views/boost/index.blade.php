...
<div class="max-w-7xl mx-auto px-4 py-8">
                ...
                @if($preselectedItem)
                    <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 mt-4 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <span class="text-emerald-700 dark:text-emerald-400">"{{ $preselectedItem->name }}" — Choisissez un type de boost pour cet article.</span>
                        </div>
                    </div>
                @endif
            @endif
        </div>
        <a href="{{ route('boost.dashboard') }}" class="inline-flex items-center gap-2 px-5 py-2.5 border border-primary text-primary font-medium rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors">
    ...
    <div class="rounded-2xl border p-6 mb-8 flex flex-col sm:flex-row sm:items-center gap-4 {{ $isLowBalance ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-800' : 'bg-primary-50 dark:bg-primary-900/10 border-primary-100 dark:border-primary-800' }}">
        <div class="flex items-center gap-4 flex-grow">
            <div class="w-12 h-12 rounded-xl {{ $isLowBalance ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400' }} flex items-center justify-center text-xl flex-shrink-0">
        ...
        <a href="#" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-white transition-colors flex-shrink-0 {{ $isLowBalance ? 'bg-red-600 hover:bg-red-700' : 'bg-primary hover:bg-primary-600' }}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-200 flex flex-col" data-boost-type="{{ $boostType->id }}">
            <div class="p-6 text-white relative" style="background: {{ $boostType->color ?? '#3B82F6' }};">
                <div class="flex justify-between items-center">
                    <h3 class="text-xl font-bold flex items-center gap-3">
                        <i class="{{ $boostType->icon ?? 'fas fa-star' }}"></i>
            ...
            <div class="p-6 flex-grow flex flex-col">
                <p class="text-gray-500 dark:text-gray-400 mb-4">{{ $boostType->description }}</p>
                ...
                <div class="mb-4 space-y-2">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm text-gray-700 dark:text-gray-300">+{{ number_format($boostType->price_per_day, 0, ',', ' ') }} {{ $currencySymbol }}</span>
                    </div>
                    @endif
                    <div class="flex justify-between items-center">
            ...
            <div class="px-6 pb-6">
                <button class="w-full bg-primary hover:bg-primary-600 text-white font-medium py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 select-boost-btn" data-boost-type-id="{{ $boostType->id }}">
