                        ...
                        <div class="mb-3">
                            ...
                            <select name="order_id" id="order_id" class="form-select">
                                ...
                                @foreach($orders as $order)
                                    <option value="{{ $order->id }}" {{ old('order_id', $review->order_id) == $order->id ? 'selected' : '' }}>#{{ $order->id }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="item_id" id="item_id" class="form-select">
                                ...
                                @foreach($items as $item)
                                    <option value="{{ $item->id }}" {{ old('item_id', $review->item_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="reviewer_id" id="reviewer_id" class="form-select" required>
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('reviewer_id', $review->reviewer_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="seller_id" id="seller_id" class="form-select" required>
                                <option value="">-- Sélectionner --</option>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('seller_id', $review->seller_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="rating" id="rating" class="form-select" required>
                                <option value="">-- Sélectionner --</option>
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('rating', $review->rating) == $i ? 'selected' : '' }}>{{ $i }} étoile{{ $i > 1 ? 's' : '' }}</option>
                                @endfor
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="comment" class="form-label">Commentaire</label>
                            <textarea name="comment" id="comment" class="form-control" rows="3" maxlength="1000">{{ old('comment', $review->comment) }}</textarea>
                        </div>
