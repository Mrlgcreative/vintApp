            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 lg:gap-6">
                ...
                @forelse($categories as $index => $category)
                    <a href="{{ route('items.index', ['category' => $category->id]) }}" 
                       class="group relative bg-white hover:bg-gradient-to-br {{ $colors[$index % count($colors)] }} border-2 border-transparent hover:border-opacity-50 rounded-2xl lg:rounded-3xl p-6 lg:p-8 text-center transition-all duration-300 hover:shadow-xl hover:shadow-black/5 hover:-translate-y-2">
                        ...
                        <div class="space-y-1">
                            ...
                            <p class="text-xs lg:text-sm text-gray-500 group-hover:text-gray-600">
                                {{ $category->items_count ?? rand(15, 150) }} articles
                            </p>
                        </div>
