...
<div class="min-h-screen pb-20">
            ...
            <div class="relative overflow-hidden rounded-3xl shadow-lg hero-gradient px-8 py-12 md:px-16 md:py-20">
                <div class="grid md:grid-cols-2 gap-8 items-center">
                    <div class="text-white space-y-8">
                        ...
                        <div class="flex flex-wrap gap-4">
                            <a href="{{ route('items.index') }}" class="group px-8 py-4 bg-white text-violet-600 rounded-full font-semibold hover-lift inline-flex items-center gap-2 shadow-lg hover:shadow-xl transition-all duration-300">
                                Explorer maintenant
    ...
    <section id="categories" class="max-w-7xl mx-auto px-4 py-6 fade-in-up">
        ...
        <div class="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
            @foreach($categories as $category)
                ...
                @endphp
                <a href="{{ route('items.index', ['category' => $category->id]) }}" class="category-card p-3 sm:p-4 hover:shadow-lg transition-all duration-300">
                    @if($catImg && Storage::disk('public')->exists($catImg))
                        <img src="{{ Storage::url($catImg) }}" alt="{{ $category->name }}" class="category-img" />
                    @else
                    ...
                    <div class="text-center">
                        <p class="category-name text-xs sm:text-sm">{{ $category->name }}</p>
                        <p class="category-sub text-xs">{{ $category->items_count ?? 0 }} articles</p>
                    </div>
    ...
    <section class="max-w-7xl mx-auto px-4 py-4">
        <div class="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
            <a href="{{ route('items.index') }}" class="filter-chip {{ !request('sort') ? 'active' : '' }} inline-flex items-center gap-2">
                <span class="text-base">⭐</span>
                Tous
            </a>
            <a href="{{ route('items.index', ['sort' => 'recent']) }}" class="filter-chip {{ request('sort') === 'recent' ? 'active' : '' }} inline-flex items-center gap-2">
                <span class="text-base">🆕</span>
                Nouveautés
            </a>
            <a href="{{ route('items.index', ['sort' => 'popular']) }}" class="filter-chip {{ request('sort') === 'popular' ? 'active' : '' }} inline-flex items-center gap-2">
                <span class="text-base">🔥</span>
                Populaires
            </a>
            <a href="{{ route('items.index', ['sort' => 'price_low']) }}" class="filter-chip {{ request('sort') === 'price_low' ? 'active' : '' }} inline-flex items-center gap-2">
                <span class="text-base">🏷️</span>
                Prix croissant
            </a>
            <a href="{{ route('items.index', ['sort' => 'price_high']) }}" class="filter-chip {{ request('sort') === 'price_high' ? 'active' : '' }} inline-flex items-center gap-2">
                <span class="text-base">💎</span>
                Prix décroissant
            </a>
        </div>
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-violet-600 via-purple-600 to-blue-600 p-8 md:p-12 text-center">
            ...
            <div class="relative z-10 max-w-3xl mx-auto text-white">
                ...
                <div class="flex flex-wrap gap-4 justify-center">
                    <a href="{{ route('items.index') }}" class="px-8 py-4 bg-white text-violet-600 rounded-full font-semibold hover:bg-gray-100 transition-all duration-300 hover:scale-105 shadow-lg">
                        Commencer à acheter
                    </a>
                    <a href="{{ route('items.create') ?? '#' }}" class="px-8 py-4 bg-transparent border-2 border-white text-white rounded-full font-semibold hover:bg-white hover:text-violet-600 transition-all duration-300">
                        Vendre mes articles
                    </a>
                </div>
            </div>
