...
class CinetPay
{
    ...
    public function __construct($site_id, $apikey, $mode = "PROD", $version = 'v2', $params = null)
    {
        ...
        // MODIFICATION: Force l'utilisation de l'API production car sandbox est inaccessible
        // CinetPay accepte les identifiants de test sur l'API production
        if ($mode == "PROD") {
            $this->_use_sandbox = false;
            $this->_use_ssl = true;
        } else {
            // En mode TEST, on utilise quand même l'API production (pas sandbox)
            // car api.sandbox.cinetpay.com est souvent inaccessible
            $this->_use_sandbox = false; // Changé de true à false
            $this->_use_ssl = true;
        }
    ...
    private function getPaySignatureArray()
    {
        ...
        if (!empty($this->_cfg_notify_url)) $dataArray['notify_url'] = $this->_cfg_notify_url;
        if (!empty($this->_cfg_return_url)) $dataArray['return_url'] = $this->_cfg_return_url;
        if (!empty($this->_cfg_cancel_url)) $dataArray['cancel_url'] = $this->_cfg_cancel_url;
    ...
    public function setTransId($id)
    {
        ...
        // Initialiser automatiquement la date de transaction si elle n'est pas définie
        if (empty($this->_cfg_cpm_trans_date)) {
            $this->_cfg_cpm_trans_date = date('YmdHis');
        }
